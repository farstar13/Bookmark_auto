...
function getBldLabel(a) {
  return typeof a.stage !== 'undefined' ? a.stages[a.stage || 0].label : a.label
}

function bldLabelCmp(a, b) {
  var aLabel = typeof a.stage !== 'undefined' ? a.stages[a.stage || 0].label : a.label
  var bLabel = typeof b.stage !== 'undefined' ? b.stages[b.stage || 0].label : b.label
...
function buildingCalculator() {
  ...
  if (gamePage.spaceTab.visible) {
    ...
    buffer = [];
    var space = gamePage.space.planets.slice(0);
    space.sort(function(a, b){return a.title.localeCompare(b.title)});
    for (var i = 0; i < space.length; i++) {
      if (space[i].unlocked) {
        var p_buildings = space[i].buildings.slice(0);
        for (var j = 0; j < p_buildings.length; j++) {
          if (p_buildings[j].unlocked) {
            buffer.push(p_buildings[j])
          }
        }
      }
    }
    buffer.sort(function(a, b){return a.title.localeCompare(b.title)});
    for (var k = 0; k < buffer.length; k++) {
      result += '<option value="space_'+buffer[k].name+'">'+buffer[k].title+'</option>';
    }
  }
